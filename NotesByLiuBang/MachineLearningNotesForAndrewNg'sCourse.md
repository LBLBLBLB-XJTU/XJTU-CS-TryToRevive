...
但是这个奖励如何定义呢：我们进一步定义奖励为过程中所有的奖励的和，在加上一个折现系数。对于第一步奖励不变，第二步的奖励则要乘上折现系数，第三步则要乘上折现系数的平方，以此类推。折现系数是一个0到1的常数。这对负的奖励同样有效。

#### 5.2强化学习的决策与策略

我们用一个函数，输入当前的状态，输出选择的动作。如选择最近的，选择最大值的等等。

#### 5.3状态动作值函数

状态动作值函数以状态和一个可以的行为的作为输入，输出的是执行这个行为后，最优的结果。选择最优的结果的动作即为要选择的动作

#### 5.4Bellman方程

除了状态动作值函数$Q(s,a)$，我们再加上某个状态的奖励值函数$R(s)$,以及执行一个动作后的状态和行为$s',a'$,则有Bellman方程：$Q(s,a) = R(s) + \gamma max_{a'}Q(s',a')$.这实际就是对折现系数的进一步的表示。

对于随机环境的情况下，我们用期望进行代替：$Q(s,a) = R(s) + \gamma E[max_{a'}Q(s',a')]$.

### 6大规模强化学习

#### 6.1连续状态空间

上面我们讨论的是离散的状态空间，只有几个状态，进一步的，一个状态可以是一个向量，向量内的内容可以是若干连续的值

#### 6.2学习状态值函数

我们希望训练一个神经网络来帮助我们进行决断，神经网络的输入将会是状态和行为，输出的将会是Q。基于此我们希望能够利用监督学习来训练网络。考虑Bellman方程，我们随机的进行操作，得到元组对$y^{(i)} = R(s^{(i)} + \gamma max_{a'}Q(s'^{(i)},a;)$，由此进行训练.

#### 6.3网络的优化

在之前的网络中，输入时我们需要多个网络来分别对一种状态下的多个行为进行预测，这非常低效。我们将输出设定为基于状态的考虑行为的结果，由此，可以用一个网络来实现所有的预测

#### 6.4$\epsilon$-贪婪算法

即以某个机率选择哪个最优的行为，以剩余的概率随机进行选择，由此可以避免一些局部最优的情况。

#### 6.5小批量与软更新

小批量选择训练集的一个子集来进行训练，由此可以加快训练

软更新选择在算法进行更新参数时，不是直接的更新为新值，而是新值和旧值进行加权，这可以是算法更加收敛
